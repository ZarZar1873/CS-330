...
int main()
{
	...
	// --------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "3-5 Milestone: Beginning a 3D Scene", NULL, NULL);
	if (window == NULL)
	{
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindTexture(GL_TEXTURE_2D, frostedGlass);
		model = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first		
		model = glm::scale(glm::vec3(2.0f, 3.0f, 1.0f));
		model = glm::translate(model, glm::vec3(0.5f, 0.35f, 0.0f));
